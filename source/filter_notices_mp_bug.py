...
def applyCNN():
    ...
    for dirn in dirList:
        ...
        for file in fileList:
            ...
            all_subimages.append([file,dirn])
    ...
    processs=[]
    count=0
    for file,dirn in all_subimages:
        count +=1
        CPU_USED+=1              
        if CPU_USED<=CPU_COUNT: 
            process=mp.Process(target=check_for_notice,args=(file,dirn))
            processs.append(process)
            print("starting process : %s" % process)
        if CPU_USED==CPU_COUNT or count==len(all_subimages):
...
def check_for_notice(file,dirn):
    print("dirn,file",dirn,file)
