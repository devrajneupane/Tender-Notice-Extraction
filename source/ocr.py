...
def is_tender(folder,img):
    ...
    if strike >=1:
        ...
        try:
            os.mkdir(path.parent.joinpath("Tender",date))
            
        except FileExistsError:
            pass
        try:
            os.mkdir(path.parent.joinpath("Tender",date,folder[:-11]))
        except FileExistsError:
            pass
        
        cv2.imwrite(str(path.parent.joinpath("Tender",date,folder[:-11],img)), image)
